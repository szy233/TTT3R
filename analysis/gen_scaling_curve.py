...
# Color palette — baselines gray-ish, ours vivid
C_CUT3R  = '#AAAAAA'   # gray for baseline
C_TTT3R  = '#D4877F'   # muted red
C_CONST  = '#E6AB02'   # gold
C_BRAKE  = '#5BAA5B'   # green
C_DDD3R  = '#3182BD'   # blue
...
# ScanNet relpose scaling curve (20 points, 5 methods)
...
SCANNET_DATA = {
    ...
    r'DDD3R$_{\rm const}$': [0.033, 0.072, 0.105, 0.141, 0.149, 0.169, 0.181, 0.192, 0.199, 0.214,
                           0.222, 0.233, 0.237, 0.242, 0.256, 0.261, 0.268, 0.272, 0.278, 0.283],
    r'DDD3R$_{\rm brake}$': [0.036, 0.080, 0.117, 0.155, 0.166, 0.185, 0.191, 0.211, 0.229, 0.282,
                           0.236, 0.242, 0.237, 0.246, 0.259, 0.255, 0.262, 0.261, 0.260, 0.261],
    r'DDD3R$_{\rm ortho}$': [0.040, 0.099, 0.185, 0.246, 0.289, 0.321, 0.344, 0.382, 0.395, 0.409,
                           0.402, 0.410, 0.422, 0.435, 0.453, 0.465, 0.472, 0.477, 0.480, 0.488],
}
...
TUM_DATA = {
    ...
    r'DDD3R$_{\rm const}$': [0.012, 0.017, 0.021, 0.027, 0.033, 0.037, 0.042, 0.050, 0.058, 0.061, 0.068, 0.079],
    r'DDD3R$_{\rm brake}$': [0.012, 0.016, None, 0.024, 0.031, 0.033, 0.037, 0.043, 0.049, 0.053, 0.057, 0.063],
    r'DDD3R$_{\rm ortho}$': [0.012, 0.015, 0.019, 0.025, 0.032, 0.033, 0.037, 0.040, 0.045, 0.049, 0.053, 0.055],
}
...
KITTI_VD_DATA = {
    ...
    r'DDD3R$_{\rm const}$': [0.0908, 0.0963, 0.0932, 0.0936, 0.0954, 0.0979, 0.1008, 0.1028, 0.1054, 0.1065],
    r'DDD3R$_{\rm brake}$': [0.0906, 0.0949, 0.0927, 0.0930, 0.0946, 0.0969, 0.0998, 0.1020, 0.1050, 0.1063],
    r'DDD3R$_{\rm ortho}$': [0.0913, 0.0992, 0.0962, 0.0953, 0.0956, 0.0969, 0.0991, 0.1005, 0.1027, 0.1033],
}
...
BONN_VD_DATA = {
    ...
    r'DDD3R$_{\rm const}$': [0.0660, 0.0567, 0.0582, 0.0608, 0.0615, 0.0710, 0.0702, 0.0700, 0.0692, 0.0687],
    r'DDD3R$_{\rm brake}$': [0.0643, 0.0556, 0.0568, 0.0592, 0.0596, 0.0685, 0.0679, 0.0679, 0.0669, 0.0661],
    r'DDD3R$_{\rm ortho}$': [0.0647, 0.0581, 0.0596, 0.0597, 0.0606, 0.0705, 0.0701, 0.0700, 0.0690, 0.0678],
}
...
def plot_scaling_panel(ax, frames, data, ylabel, title, show_legend=False,
    ...
    method_style = {
        'CUT3R':              (C_CUT3R, 'o', '--', 1.0),
        'TTT3R':              (C_TTT3R, 's', '--', 1.0),
        r'DDD3R$_{\rm const}$': (C_CONST, 'D', '-', 1.2),
        r'DDD3R$_{\rm brake}$': (C_BRAKE, '^', '-', 1.4),
        r'DDD3R$_{\rm ortho}$': (C_DDD3R, 'v', '-', 1.4),
    }

    for method, values in data.items():
        color, marker, ls, lw = method_style[method]
        ...
        ax.plot(x, y, ls, color=color, marker=marker, markersize=3.0,
...
def fig_relpose_scaling():
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(NEURIPS_TEXTWIDTH, 2.2))
    ...
    # ScanNet: annotate endpoint growth + ranking reversal
    fs = 5.0
    ax1.annotate('2.4$\\times$', xy=(1000, 0.817), xytext=(5, 0),
                 textcoords='offset points', fontsize=fs, color=C_CUT3R, va='center')
    ax1.annotate('1.7$\\times$', xy=(1000, 0.261), xytext=(5, 0),
                 textcoords='offset points', fontsize=fs, color=C_BRAKE, va='center')

    # TUM: annotate endpoint
    ax2.annotate('3.0$\\times$', xy=(1000, 0.166), xytext=(5, 0),
                 textcoords='offset points', fontsize=fs, color=C_CUT3R, va='center')
    ax2.annotate('2.2$\\times$', xy=(1000, 0.055), xytext=(5, 0),
                 textcoords='offset points', fontsize=fs, color=C_DDD3R, va='center')
