"""
B3 Geometric Consistency Gate Ablation
=======================================

Compares depth error for update types:
  cut3r          — baseline
  ttt3r          — TTT3R attention gate
  cut3r_geogate  — geometric consistency gate on state update
  ttt3r_geogate  — TTT3R + geometric consistency gate

Also sweeps geo_gate_tau for cut3r_geogate.
...
def parse_args():
    p = argparse.ArgumentParser(description="B3 Geo Gate Ablation")
    ...
    p.add_argument("--output_dir", type=str, default="analysis_results/geogate_ablation")
...
def main():
    ...
    # ── Summary ──
    lines = ["=" * 80, "B3 Geometric Consistency Gate Ablation", "=" * 80, ""]
    ...
    lines.append(f"{'Config':<25s} {'ALL err':>12s} {'ScanNet':>12s} {'TUM':>12s} {'d_ALL%':>8s}")
    lines.append("-" * 72)
    ...
    summary_path = os.path.join(args.output_dir, "geogate_ablation_summary.txt")
    ...
    ax.set_title("B3 Geo Consistency Gate Ablation")
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(args.output_dir, "geogate_ablation.png"),
