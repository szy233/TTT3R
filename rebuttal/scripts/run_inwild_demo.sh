...
for clip in westlake taylor; do
  for m in cut3r ttt3r ddd3r_brake ddd3r; do
    ...
    setsid $PY demo.py \
      ...
      --model_update_type $m --frame_interval 1 --reset_interval 1000 \
    ...
    for _ in $(seq 1 150); do
      sleep 4
      if /bin/grep -q "Launching point cloud viewer" $LOG; then break; fi
      if ! kill -0 $pid 2>/dev/null; then break; fi
    done
    pkill -P $pid 2>/dev/null
    kill -9 $pid 2>/dev/null
    sleep 3
    echo "    saved: $(/bin/ls $OUT 2>/dev/null | wc -l) files" >> $LOG
  done
done
