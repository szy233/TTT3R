...
- **d_t**: EMA drift direction（EMA 后 normalize），`d_t = norm(β_ema · d_{t-1} + (1-β_ema) · δ̂_t)`
- **δ̂_t**: unit-normalized delta，`δ_t / ‖δ_t‖`
- **e_t**: drift energy（代码中使用 EMA 平滑值 ē_t），`⟨δ̂_t, d̂_t⟩²`
...
### Steep Adaptive 公式
```
e_t = ⟨δ̂_t, d̂_t⟩²                              # per-token instantaneous drift energy
ē_t = β_ema · ē_{t-1} + (1-β_ema) · e_t          # EMA-smoothed drift energy (code实际使用)
w_t = ē_t^γ                                       # weight toward isotropic
...
- ē_t 高 (strong drift) → w 大 → 退化为 isotropic → 保留有用 drift
- ē_t 低 (weak drift) → w 小 → 全方向分解 → 压制有害 drift
...
| 评测类型 | 数据集 | 脚本 |
|---------|--------|------|
| Camera Pose | ScanNet, TUM, Sintel, KITTI odom | `eval/relpose/launch.py` |
| Video Depth | KITTI, Bonn, Sintel | `eval/video_depth/launch.py` → `eval/video_depth/eval_depth.py` |
...
# e.g.: bash eval/run_ddd3r_eval.sh 0 tum_s1_1000 ddd3r_constant_p33   # alpha=0.33

# Portable: set DDD3R_PYTHON env var to override python path
# DDD3R_PYTHON=/path/to/python bash eval/run_ddd3r_eval.sh ...
...
### Video Depth 评测协议
- **两步流程**: `launch.py` 跑推理存 `.npy` → `eval_depth.py` 计算 abs_rel 等指标
- **Alignment**: `scale&shift`（L1-optimal，默认，与 CUT3R/MonST3R 一致）
- `run_ddd3r_eval.sh` 自动执行两步。可通过 `DDD3R_DEPTH_ALIGN=scale` 切换为 median-scale
- **聚合**: pixel-count-weighted average（非 scene-mean）

### Relpose ATE 评测协议
- **Alignment**: Sim(3) with `correct_scale=True`（evo 标准）
- **Metric**: RMSE of translation error
- **聚合**: 算术 mean over valid scenes

### Constant Dampening per-dataset 最优 α
| 数据集 | 最优 α | 命令 |
|--------|--------|------|
| TUM | 0.33 | `ddd3r_constant_p33` |
| ScanNet | 0.5 | `ddd3r_constant` |
